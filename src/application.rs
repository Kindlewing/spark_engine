...
pub fn run(event_loop: glutin::event_loop::EventLoop<()>, display: glium::Display) {
    ...
    let vertex_buffer = glium::VertexBuffer::new(&display, &shape).expect("Error creating buffer"); 
    ...
    event_loop.run(move |ev, _, control_flow| {
        let color: Color = Color::new(33., 33., 33.);
        let mut target = display.draw();

        target.clear_color(color.b, color.g, color.b, 1.0);
